...
public class CrabEggBlock extends Block
{
	...
	private void tryBreakEgg(World world, BlockState state, BlockPos pos, Entity entity, int inverseChance) {
		if (this.breaksEgg(world, entity)) {
			if (!world.isClient && world.random.nextInt(inverseChance) == 0 && state.isOf(LostLegendsBlocks.CRAB_EGG.get())) {
		...
		if (this.shouldHatchProgress(world) && isSuitableBelow(world, pos)) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < state.get(EGGS); ++j) {
					world.syncWorldEvent(2001, pos, Block.getRawIdFromState(state));
					CrabEntity crab = LostLegendsEntityTypes.CRAB.get().create(world);
	...
	public static boolean isSuitableBelow(BlockView world, BlockPos pos) {
		return world.getBlockState(pos.down()).isIn(LostLegendsTags.CRAB_BURROW_SPOT_BLOCKS);
	}
