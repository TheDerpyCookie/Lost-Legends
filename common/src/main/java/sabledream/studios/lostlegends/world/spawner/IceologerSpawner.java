package sabledream.studios.lostlegends.world.spawner;
...
public final class IceologerSpawner implements SpecialSpawner
{
	...
	public int spawn(ServerWorld world, boolean spawnMonsters, boolean spawnAnimals) {
		if (
			spawnMonsters == false
			|| LostLegends.getConfig().enableIceologerSpawn == false
		) {
			return 0;
		}
		...
		if (world.getBiome(mutable).isIn(LostLegendsTags.HAS_ICEOLOGER) == false) {
			return 0;
		}
		...
		if (
			SpawnHelper.isClearForSpawn(world, mutable, blockState, blockState.getFluidState(), LostLegendsEntityTypes.ICEOLOGER.get()) == false
			|| PatrolEntity.canSpawn(LostLegendsEntityTypes.ICEOLOGER.get(), world, SpawnReason.PATROL, mutable, random) == false
		) {
			return 0;
		}

		var iceologer = LostLegendsEntityTypes.ICEOLOGER.get().create(world);
