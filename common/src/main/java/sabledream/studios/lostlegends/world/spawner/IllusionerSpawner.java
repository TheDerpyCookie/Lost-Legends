...
public final class IllusionerSpawner implements SpecialSpawner
{
	...
	public int spawn(ServerWorld world, boolean spawnMonsters, boolean spawnAnimals) {
		if (
			spawnMonsters == false
			|| LostLegends.getConfig().enableIllusioner == false
			|| LostLegends.getConfig().enableIllusionerSpawn == false
		) {
			return 0;
		}
		...
		if (world.getBiome(mutable).isIn(LostLegendsTags.HAS_ILLUSIONER) == false) {
			return 0;
		}
