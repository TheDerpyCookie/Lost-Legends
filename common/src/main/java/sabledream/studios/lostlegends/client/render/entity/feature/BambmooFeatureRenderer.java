...
public class BambmooFeatureRenderer<T extends BambmooEntity> extends FeatureRenderer<T,CowEntityModel<T>>
{
	private final BlockRenderManager blockRenderer;

	public BambmooFeatureRenderer(FeatureRendererContext<T, CowEntityModel<T>> renderLayerParent, BlockRenderManager BlockRenderManager) {
	...
	public void render(MatrixStack poseStack, VertexConsumerProvider multiBufferSource, int light, T entity, float f, float g, float h, float j, float k, float l) {
		if (!entity.isBaby()) {
			...
			if (!entity.isInvisible() || bl) {
				...
				BlockState blockState = Blocks.BAMBOO.getDefaultState().with(BambooBlock.LEAVES, BambooLeaves.SMALL);
