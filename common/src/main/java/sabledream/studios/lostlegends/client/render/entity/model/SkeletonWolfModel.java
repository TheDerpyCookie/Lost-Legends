...
public class SkeletonWolfModel<T extends SkeletonWolfEntity> extends TintableAnimalModel<T>
{
	...
	public void setAngles(T entity, float limbAngle, float limbDistance, float animationProgress, float headyRot, float headxRot) {
		head.pitch = headxRot * ((float) Math.PI / 180F);
		head.yaw = headyRot * ((float) Math.PI / 180F);
