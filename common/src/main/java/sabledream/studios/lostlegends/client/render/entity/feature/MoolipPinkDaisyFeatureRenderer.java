...
public class MoolipPinkDaisyFeatureRenderer<T extends MoolipEntity> extends FeatureRenderer<T,CowEntityModel<T>>
{
	...
	public MoolipPinkDaisyFeatureRenderer(FeatureRendererContext<T, CowEntityModel<T>> renderLayerParent, BlockRenderManager BlockRenderManager) {
	...
	public void render(MatrixStack poseStack, VertexConsumerProvider multiBufferSource, int light, T entity, float f, float g, float h, float j, float k, float l) {
		if (!entity.isBaby()) {
			...
			if (!entity.isInvisible() || bl) {
				...
				BlockState blockState = LostLegendsBlocks.PINKDAISY.get().getDefaultState();
				...
				renderPinkDaisyBlock(poseStack, multiBufferSource, light, bl, blockState, coords, bakedModel);
				poseStack.pop();
				poseStack.push();
	...
	private void renderPinkDaisyBlock(MatrixStack poseStack, VertexConsumerProvider multiBufferSource, int i, boolean bl, BlockState blockState, int j, BakedModel bakedModel) {
		if (bl) {
