package sabledream.studios.lostlegends.config.omegaconfig;
...
public final class OmegaConfig
{
		...
		try {
			...
			if (!configExists(config)) {
				...
				try {
					...
					LostLegends.getLogger().error(e.toString());
					LostLegends.getLogger().info(String.format("Encountered an error while reading %s config, falling back to default values.", config.getName()));
					LostLegends.getLogger().info(String.format("If this problem persists, delete the config file %s and try again.", config.getName() + "." + config.getExtension()));
				}
			}
	...
	public static <T extends Config> void writeConfig(Class<T> configClass, T instance) {
		...
		try {
			...
			LostLegends.getLogger().error(ioException.toString());
			LostLegends.getLogger().info(String.format("Write error, using default values for config %s.", configClass));
		}
	}
