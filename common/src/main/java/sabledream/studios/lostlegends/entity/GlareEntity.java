...
import sabledream.studios.lostlegends.entity.ai.pathing.CachedPathHolder;
import sabledream.studios.lostlegends.entity.animation.AnimatedEntity;
import sabledream.studios.lostlegends.init.LostLegendsCriteria;
...
import sabledream.studios.lostlegends.util.particle.ParticleSpawner;
...
public final class GlareEntity extends TameableEntity implements Flutterer, AnimatedEntity
{
	...
	static {
		...
		PICKABLE_FOOD_FILTER = (itemEntity) -> {
			return itemEntity.getStack().isIn(LostLegendsTags.GLARE_FOOD_ITEMS) && itemEntity.isAlive() && itemEntity.cannotPickup() == false;
		};
	}
	...
	public static boolean canSpawn(
		...
		boolean isRelatedBlock = blockState.isIn(LostLegendsTags.GLARES_SPAWNABLE_ON);
	...
	@Override
	public void tick() {
		if (LostLegends.getConfig().enableGlare == false) {
			this.discard();
		}
	...
	protected SoundEvent getAmbientSound() {
		return LostLegendsSoundEvents.ENTITY_GLARE_AMBIENT.get();
	}
	...
	private SoundEvent getGrumpinessSound() {
		return LostLegendsSoundEvents.ENTITY_GLARE_GRUMPINESS.get();
	}
	...
	private SoundEvent getGrumpinessShortSound() {
		return LostLegendsSoundEvents.ENTITY_GLARE_GRUMPINESS_SHORT.get();
	}
	...
	private SoundEvent getRustleSound() {
		return LostLegendsSoundEvents.ENTITY_GLARE_RUSTLE.get();
	}
	...
	private SoundEvent getShakeSound() {
		return LostLegendsSoundEvents.ENTITY_GLARE_SHAKE.get();
	}
	...
	public SoundEvent getEatSound(ItemStack stack) {
		return LostLegendsSoundEvents.ENTITY_GLARE_EAT.get();
	}
	...
	protected SoundEvent getHurtSound(DamageSource source) {
		return LostLegendsSoundEvents.ENTITY_GLARE_HURT.get();
	}
	...
	protected SoundEvent getDeathSound() {
		return LostLegendsSoundEvents.ENTITY_GLARE_DEATH.get();
	}
	...
	public GlobalPos getGlowBerriesPos() {
		return this.getBrain().getOptionalMemory(LostLegendsMemoryModuleTypes.GLARE_GLOW_BERRIES_POS.get()).orElse(null);
	}
	...
	public GlobalPos getDarkSpotPos() {
		return this.getBrain().getOptionalMemory(LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_POS.get()).orElse(null);
	}
	...
	public void setOwner(PlayerEntity owner) {
		...
		if (owner instanceof ServerPlayerEntity) {
			LostLegendsCriteria.TAME_GLARE.get().trigger((ServerPlayerEntity) owner, this);
		}
	}
	...
	public PassiveEntity createChild(ServerWorld serverWorld, PassiveEntity entity) {
		GlareEntity glareEntity = LostLegendsEntityTypes.GLARE.get().create(serverWorld);
