...
import sabledream.studios.lostlegends.mixin.LimbAnimatorAccessor;
...
public final class CopperGolemEntity extends GolemEntity implements AnimatedEntity
{
	...
	protected SoundEvent getHurtSound(DamageSource source) {
		return LostLegendsSoundEvents.ENTITY_COPPER_GOLEM_HURT.get();
	}

	@Override
	protected SoundEvent getDeathSound() {
		return LostLegendsSoundEvents.ENTITY_COPPER_GOLEM_DEATH.get();
	}
	...
	protected void playStepSound(
		...
		this.playSound(LostLegendsSoundEvents.ENTITY_COPPER_GOLEM_STEP.get(), blockSoundGroup.getVolume() * 0.15F, this.getSoundPitch());
	}
	...
	private boolean tryToInteractMobWithCopperIngot(
		...
		this.playSound(LostLegendsSoundEvents.ENTITY_COPPER_GOLEM_REPAIR.get(), 1.0F, this.getSoundPitch() - 1.0F);
	...
	@Override
	public void tick() {
		if (LostLegends.getConfig().enableCopperGolem == false) {
			this.discard();
		}
	...
	public void startSpinHeadAnimation() {
		...
		this.playSound(LostLegendsSoundEvents.ENTITY_COPPER_GOLEM_HEAD_SPIN.get(), 1.0F, this.getSoundPitch() - 1.5F);
	...
	public void setOxidationLevel(Oxidizable.OxidationLevel oxidationLevel) {
		...
		if (this.isOxidized() && this.getBrain().getOptionalMemory(LostLegendsMemoryModuleTypes.COPPER_GOLEM_IS_OXIDIZED.get()).isEmpty()) {
			this.getBrain().remember(LostLegendsMemoryModuleTypes.COPPER_GOLEM_IS_OXIDIZED.get(), true);
			this.becomeStatue();
		} else if (this.isOxidized() == false && this.getBrain().getOptionalMemory(LostLegendsMemoryModuleTypes.COPPER_GOLEM_IS_OXIDIZED.get()).isPresent()) {
			this.getBrain().forget(LostLegendsMemoryModuleTypes.COPPER_GOLEM_IS_OXIDIZED.get());
			this.becomeEntity();
		}
	}
	...
	public GlobalPos getButtonPos() {
		return this.getBrain().getOptionalMemory(LostLegendsMemoryModuleTypes.COPPER_GOLEM_BUTTON_POS.get()).orElse(null);
	}

	public boolean isButtonValidToBePressed(BlockPos pos) {
		return this.getWorld().getBlockState(pos).isIn(LostLegendsTags.COPPER_BUTTONS);
	}
