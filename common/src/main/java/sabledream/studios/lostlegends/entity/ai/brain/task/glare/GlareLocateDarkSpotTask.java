...
public final class GlareLocateDarkSpotTask extends MultiTickTask<GlareEntity>
{
	public GlareLocateDarkSpotTask() {
		super(Map.of(
			LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_LOCATING_COOLDOWN.get(), MemoryModuleState.VALUE_ABSENT,
			LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_POS.get(), MemoryModuleState.VALUE_ABSENT
		), 1);
	}
	...
	protected void run(ServerWorld world, GlareEntity glare, long time) {
		...
		glare.getBrain().remember(LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_POS.get(), GlobalPos.create(registryKey, darkSpotPos));
	}
