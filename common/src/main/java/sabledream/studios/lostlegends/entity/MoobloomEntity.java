...
public class MoobloomEntity extends CowEntity implements Shearable
{
	...
	public ActionResult interactMob(
		...
		if (moobloomVariant != null && moobloomVariant != this.getVariant()) {
			this.setVariant(moobloomVariant);
			this.playSound(LostLegendsSoundEvents.ENTITY_MOOBLOOM_CONVERT.get(), 2.0F, 1.0F);
	...
	public MoobloomEntity createChild(
		...
		MoobloomEntity moobloom = LostLegendsEntityTypes.MOOBLOOM.get().create(serverWorld);
			...
			if (blockUnderneath == Blocks.GRASS_BLOCK && this.getWorld().isAir(this.getBlockPos())) {
				...
				if (MoobloomVariantManager.MOOBLOOM_VARIANT_MANAGER.getMoobloomVariants().size() == 1) {
					...
					if (flowerChance >= 0 && flowerChance < 40) {
						this.getWorld().setBlockState(this.getBlockPos(), LostLegendsBlocks.BUTTERCUP.get().getDefaultState());
	...
	@Override
	public void tick() {
		if (LostLegends.getConfig().enableMoobloom == false) {
			this.discard();
		}
