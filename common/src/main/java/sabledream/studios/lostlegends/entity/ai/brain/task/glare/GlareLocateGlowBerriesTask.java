...
public final class GlareLocateGlowBerriesTask extends MultiTickTask<GlareEntity>
{
	public GlareLocateGlowBerriesTask() {
		super(Map.of(
			LostLegendsMemoryModuleTypes.GLARE_LOCATING_GLOW_BERRIES_COOLDOWN.get(), MemoryModuleState.VALUE_ABSENT,
			LostLegendsMemoryModuleTypes.GLARE_GLOW_BERRIES_POS.get(), MemoryModuleState.VALUE_ABSENT
		));
	}
	...
	protected void run(ServerWorld world, GlareEntity glare, long time) {
		...
		glare.getBrain().remember(LostLegendsMemoryModuleTypes.GLARE_GLOW_BERRIES_POS.get(), GlobalPos.create(registryKey, glowBerriesPos));
	}
