...
import sabledream.studios.lostlegends.entity.ai.brain.WildfireBrain;
...
public final class WildfireEntity extends HostileEntity
{
	...
	protected void playStepSound(BlockPos pos, BlockState state) {
		...
		this.playSound(LostLegendsSoundEvents.ENTITY_WILDFIRE_STEP.get(), blockSoundGroup.getVolume() * 0.15F, blockSoundGroup.getPitch());


	}

	public SoundEvent getShootSound() {
		return LostLegendsSoundEvents.ENTITY_WILDFIRE_SHOOT.get();
	}
	...
	public SoundEvent getShockwaveSound() {
		return LostLegendsSoundEvents.ENTITY_WILDFIRE_SHOCKWAVE.get();
	}
	...
	public SoundEvent getShieldBreakSound() {
		return LostLegendsSoundEvents.ENTITY_WILDFIRE_SHIELD_BREAK.get();
	}
	...
	protected SoundEvent getAmbientSound() {
		return LostLegendsSoundEvents.ENTITY_WILDFIRE_AMBIENT.get();
	}

	protected SoundEvent getHurtSound(DamageSource source) {
		return LostLegendsSoundEvents.ENTITY_WILDFIRE_HURT.get();
	}

	protected SoundEvent getDeathSound() {
		return LostLegendsSoundEvents.ENTITY_WILDFIRE_DEATH.get();
	}

	public SoundEvent getSummonBlazeSound() {
		return LostLegendsSoundEvents.ENTITY_WILDFIRE_SUMMON_BLAZE.get();
	}
	...
	@Override
	public void tick() {
		if (LostLegends.getConfig().enableWildfire == false) {
			this.discard();
		}
	...
	public boolean damage(
		...
		if (
			...
			|| (attacker != null && attacker.getType().isIn(LostLegendsTags.WILDFIRE_ALLIES))
		) {
			return false;
		}
