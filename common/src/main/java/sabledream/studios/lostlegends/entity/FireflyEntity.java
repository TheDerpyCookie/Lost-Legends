...
public class FireflyEntity extends PathAwareEntity implements Flutterer
{
	...
	protected void initDataTracker(DataTracker.Builder builder) {
		...
		builder.add(ILLUMINATING_TICKS, 0);
	...
	public void tickMovement() {
		if (this.isIlluminant()) {
			...
			if (this.getLuminance() == 3 && this.random.nextInt(20) == 0)
				this.getEntityWorld().addParticle(LostLegendsParticleTypes.FIREFLY_EMISSION, this.getX(), this.getY(), this.getZ(), 0.0, 0.0, 0.0);
		}
	...
	protected SoundEvent getDeathSound() {
		return LostLegendsSoundEvents.FIREFLY_DEATH.get();
	}

	@Override
	protected SoundEvent getAmbientSound() {
		return LostLegendsSoundEvents.FIREFLY_AMBIENT.get();
	}
