...
public class BarnacleSpecificSensor extends Sensor<BarnacleEntity>
{
	...
	protected void sense(ServerWorld world, BarnacleEntity barnacle) {
		...
		LivingEntity firstEntityToBeAvoided = livingTargetCache.findFirst(livingEntity -> livingEntity.getType().isIn(LostLegendsTags.BARNACLE_AVOID_TARGETS)).orElse(null);
		LivingEntity firstEntityToBeHunted = livingTargetCache.findFirst(livingEntity -> livingEntity.getType().isIn(LostLegendsTags.BARNACLE_PREY)).orElse(null);
