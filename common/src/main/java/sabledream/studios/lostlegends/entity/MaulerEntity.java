...
import sabledream.studios.lostlegends.entity.ai.goal.mauler.*;
...
public final class MaulerEntity extends PathAwareEntity implements Angerable, AnimatedEntity
{
	...
	static {
		PREY_PREDICATE_FILTER = (entity) -> {
			...
			return entity.getType().isIn(LostLegendsTags.MAULER_PREY);
		};
	...
	public static boolean canSpawn(
		...
		return serverWorldAccess.getBlockState(blockPos.down()).isIn(LostLegendsTags.MAULERS_SPAWNABLE_ON);
	}
	...
	@Override
	public void tick() {
		if (LostLegends.getConfig().enableMauler == false) {
			this.discard();
		}
	...
	private boolean tryToInteractWithEnhancedItem(PlayerEntity player, Hand hand, ItemStack itemStack) {
		...
		this.playSound(LostLegendsSoundEvents.ENTITY_MAULER_BITE.get(), 0.2F, RandomGenerator.generateFloat(0.9F, 0.95F));
	...
	protected SoundEvent getAmbientSound() {
		return LostLegendsSoundEvents.ENTITY_MAULER_GROWL.get();
	}
	...
	protected SoundEvent getHurtSound(DamageSource source) {
		return LostLegendsSoundEvents.ENTITY_MAULER_HURT.get();
	}
	...
	protected SoundEvent getDeathSound() {
		return LostLegendsSoundEvents.ENTITY_MAULER_DEATH.get();
	}
