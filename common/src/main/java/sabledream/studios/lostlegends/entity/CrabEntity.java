...
import sabledream.studios.lostlegends.block.CrabEggBlock;
...
public class CrabEntity extends AnimalEntity implements Flutterer, AnimatedEntity
{
	...
	public boolean hasNoDrag() {
		if (this.isSwimming()) {
			LostLegends.getLogger().info("swimming");
			return true;
		} else {
			return false;
		}
	...
	protected void playStepSound(
		...
		this.playSound(LostLegendsSoundEvents.ENTITY_CRAB_STEP.get(), 0.15f, 1.0f + this.random.nextFloat() * 0.2f);
	}

	@Override
	public void tick() {

		if (this.getWorld().isClient() == false && LostLegends.getConfig().enableCrab == false) {
			this.discard();
		}
	...
	private KeyframeAnimation getKeyframeAnimationByPose() {
		...
		if (this.isInPose(CrabEntityPose.IDLE.get()) && this.isMoving() == false) {
			keyframeAnimation = CrabAnimations.IDLE;
		} else if (this.isInPose(CrabEntityPose.WAVE.get())) {
	...
	public void startWaveAnimation() {
		if (this.isInPose(CrabEntityPose.WAVE.get())) {
			return;
		}

		this.setPose(CrabEntityPose.WAVE.get());
	}
	@Override
	...
	protected void onGrowUp() {
		...
		if (!this.isBaby() && this.getWorld().getGameRules().getBoolean(GameRules.DO_MOB_LOOT)) {
			this.dropItem(LostLegendsItems.CRAB_CLAW.get(), 1);
		}
	}
	...
	public static boolean canSpawn(
		...
		return world.getBlockState(pos.down()).isIn(LostLegendsTags.CRABS_SPAWNABLE_ON) && CrabEggBlock.isSuitableBelow(world, pos) && isLightLevelValidForNaturalSpawn(world, pos);
	}
	...
	public PassiveEntity createChild(ServerWorld serverWorld, PassiveEntity entity) {
		CrabEntity crab = LostLegendsEntityTypes.CRAB.get().create(serverWorld);
	...
	public GlobalPos getBurrowSpotPos() {
		return this.getBrain().getOptionalMemory(LostLegendsMemoryModuleTypes.CRAB_BURROW_POS.get()).orElse(null);
	}

	public boolean isBurrowSpotAccessible(BlockPos pos) {
		var world = this.getWorld();
		boolean isBlockSand = world.getBlockState(pos.down()).isIn(LostLegendsTags.CRAB_BURROW_SPOT_BLOCKS);
