...
import sabledream.studios.lostlegends.entity.ai.brain.task.barnacle.AquaticStrollTask;
...
public final class BarnacleBrain
{
	...
	private static void addFightActivities(Brain<BarnacleEntity> brain) {
		brain.setTaskList(
			...
			0,
			ImmutableList.of(
				 RangedApproachTask.create(0.6F),
	...
	private static void addIdleActivities(Brain<BarnacleEntity> brain) {
		brain.setTaskList(
			Activity.IDLE,
			ImmutableList.of(
				Pair.of(0,  UpdateAttackTargetTask.create(barnacle -> getAttackTarget((BarnacleEntity) barnacle))),
	...
	private static void addAvoidActivities(Brain<BarnacleEntity> brain) {
		brain.setTaskList(
			Activity.AVOID,
			ImmutableList.of(
				Pair.of(0, GoToRememberedPositionTask.createEntityBased(MemoryModuleType.AVOID_TARGET, 0.8F, 16, false))
			),
			ImmutableSet.of(
	...
	static {
		SENSORS = List.of(
			...
			SensorType.HURT_BY,
			LostLegendsMemorySensorType.BARNACLE_SPECIFIC_SENSOR.get(),
			LostLegendsMemorySensorType.BARNACLE_ATTACKABLE_SENSOR.get()
		);
