...
import sabledream.studios.lostlegends.entity.ai.brain.task.rascal.RascalFindInteractionTargetTask;
import sabledream.studios.lostlegends.entity.ai.brain.task.rascal.RascalWaitForPlayerTask;
...
public final class RascalBrain
{
	...
	private static void addCoreActivities(Brain<RascalEntity> brain) {
		brain.setTaskList(Activity.CORE,
			...
			ImmutableList.of(
				...
				new TemptationCooldownTask(LostLegendsMemoryModuleTypes.RASCAL_NOD_COOLDOWN.get())
			)
		);
	}
	...
	private static void addWaitActivities(
		...
		brain.setTaskList(
			LostLegendsActivities.RASCAL_WAIT.get(),
			10,
			ImmutableList.of(
	...
	public static void setNodCooldown(RascalEntity rascal) {
		rascal.getBrain().remember(LostLegendsMemoryModuleTypes.RASCAL_NOD_COOLDOWN.get(), NOD_COOLDOWN_PROVIDER.get(rascal.getRandom()));
		onCooldown(rascal);
	}

	public static boolean shouldRunAway(RascalEntity rascal) {
		return rascal.getBrain().getOptionalMemory(LostLegendsMemoryModuleTypes.RASCAL_NOD_COOLDOWN.get()).isPresent();
	}
	...
	static {
		...
		MEMORY_MODULES = List.of(
			...
			LostLegendsMemoryModuleTypes.RASCAL_NOD_COOLDOWN.get()
		);
