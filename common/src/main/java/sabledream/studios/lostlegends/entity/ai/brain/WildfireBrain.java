...
import sabledream.studios.lostlegends.entity.ai.brain.task.wildfire.WildfireBarrageAttackTask;
import sabledream.studios.lostlegends.entity.ai.brain.task.wildfire.WildfireShockwaveAttackTask;
import sabledream.studios.lostlegends.entity.ai.brain.task.wildfire.WildfireSummonBlazeTask;
...
public final class WildfireBrain
{
	...
	private static void addCoreActivities(Brain<WildfireEntity> brain) {
		brain.setTaskList(Activity.CORE,
			...
			ImmutableList.of(
				...
				new TemptationCooldownTask(LostLegendsMemoryModuleTypes.WILDFIRE_BARRAGE_ATTACK_COOLDOWN.get()),
				new TemptationCooldownTask(LostLegendsMemoryModuleTypes.WILDFIRE_SHOCKWAVE_ATTACK_COOLDOWN.get()),
				new TemptationCooldownTask(LostLegendsMemoryModuleTypes.WILDFIRE_SUMMON_BLAZE_COOLDOWN.get())
			)
		);
	}
	...
	public static void setBarrageAttackCooldown(WildfireEntity wildfire) {
		wildfire.getBrain().remember(LostLegendsMemoryModuleTypes.WILDFIRE_BARRAGE_ATTACK_COOLDOWN.get(), BARRAGE_ATTACK_COOLDOWN_PROVIDER.get(wildfire.getRandom()));
		onCooldown(wildfire);
	}

	public static void setShockwaveAttackCooldown(WildfireEntity wildfire) {
		wildfire.getBrain().remember(LostLegendsMemoryModuleTypes.WILDFIRE_SHOCKWAVE_ATTACK_COOLDOWN.get(), SHOCKWAVE_ATTACK_COOLDOWN_PROVIDER.get(wildfire.getRandom()));
		onCooldown(wildfire);
	}

	public static void setSummonBlazeCooldown(WildfireEntity wildfire) {
		wildfire.getBrain().remember(LostLegendsMemoryModuleTypes.WILDFIRE_SUMMON_BLAZE_COOLDOWN.get(), SUMMON_BLAZE_COOLDOWN_PROVIDER.get(wildfire.getRandom()));
		onCooldown(wildfire);
	}

	public static boolean shouldRunAway(WildfireEntity wildfire) {
		return wildfire.getBrain().getOptionalRegisteredMemory(LostLegendsMemoryModuleTypes.WILDFIRE_BARRAGE_ATTACK_COOLDOWN.get()).isPresent()
			   && wildfire.getBrain().getOptionalRegisteredMemory(LostLegendsMemoryModuleTypes.WILDFIRE_SHOCKWAVE_ATTACK_COOLDOWN.get()).isPresent()
			   && (
				   wildfire.getBrain().getOptionalRegisteredMemory(LostLegendsMemoryModuleTypes.WILDFIRE_SUMMON_BLAZE_COOLDOWN.get()).isPresent()
	...
	static {
		...
		MEMORY_MODULES = List.of(
			...
			LostLegendsMemoryModuleTypes.WILDFIRE_SUMMON_BLAZE_COOLDOWN.get(),
			LostLegendsMemoryModuleTypes.WILDFIRE_BARRAGE_ATTACK_COOLDOWN.get(),
			LostLegendsMemoryModuleTypes.WILDFIRE_SHOCKWAVE_ATTACK_COOLDOWN.get()
		);
