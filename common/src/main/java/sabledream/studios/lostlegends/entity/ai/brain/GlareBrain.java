...
import sabledream.studios.lostlegends.entity.ai.brain.task.glare.*;
...
public final class GlareBrain
{
	...
	private static void addCoreActivities(Brain<GlareEntity> brain) {
		brain.setTaskList(Activity.CORE,
			...
			ImmutableList.of(
				...
				new TemptationCooldownTask(LostLegendsMemoryModuleTypes.GLARE_LOCATING_GLOW_BERRIES_COOLDOWN.get()),
				new TemptationCooldownTask(LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_LOCATING_COOLDOWN.get())
			)
		);
	}
	...
	private static void addDarkSpotActivities(
		...
		brain.setTaskList(
			LostLegendsActivities.GLARE_SHOW_DARK_SPOT.get(),
			ImmutableList.of(
			...
			ImmutableSet.of(
				...
				Pair.of(LostLegendsMemoryModuleTypes.GLARE_IS_TAMED.get(), MemoryModuleState.VALUE_PRESENT),
				Pair.of(LostLegendsMemoryModuleTypes.GLARE_IS_IDLE.get(), MemoryModuleState.VALUE_ABSENT),
				Pair.of(LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_LOCATING_COOLDOWN.get(), MemoryModuleState.VALUE_ABSENT)
			)
		);
	}
	...
	private static void addGlowBerriesActivities(
		...
		brain.setTaskList(
			LostLegendsActivities.GLARE_EAT_GLOW_BERRIES.get(),
			ImmutableList.of(
			...
			ImmutableSet.of(
				...
				Pair.of(MemoryModuleType.TEMPTING_PLAYER, MemoryModuleState.VALUE_ABSENT),
				Pair.of(LostLegendsMemoryModuleTypes.GLARE_IS_IDLE.get(), MemoryModuleState.VALUE_ABSENT),
				Pair.of(LostLegendsMemoryModuleTypes.GLARE_LOCATING_GLOW_BERRIES_COOLDOWN.get(), MemoryModuleState.VALUE_ABSENT)
			)
		);
	}
	...
	private static void addAvoidActivities(Brain<GlareEntity> brain) {
		brain.setTaskList(
			...
			ImmutableSet.of(
				...
				Pair.of(LostLegendsMemoryModuleTypes.GLARE_IS_TAMED.get(), MemoryModuleState.VALUE_ABSENT),
				Pair.of(LostLegendsMemoryModuleTypes.GLARE_IS_IDLE.get(), MemoryModuleState.VALUE_ABSENT)
			)
		);
	}
	...
	private static void addIdleActivities(Brain<GlareEntity> brain) {
		brain.setTaskList(
			...
			ImmutableList.of(
				...
				Pair.of(1, new BreedTask(LostLegendsEntityTypes.GLARE.get())),
			...
			ImmutableSet.of(
				Pair.of(MemoryModuleType.AVOID_TARGET, MemoryModuleState.VALUE_ABSENT),
				Pair.of(LostLegendsMemoryModuleTypes.GLARE_IS_IDLE.get(), MemoryModuleState.VALUE_ABSENT),
				Pair.of(LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_LOCATING_COOLDOWN.get(), MemoryModuleState.VALUE_PRESENT),
				Pair.of(LostLegendsMemoryModuleTypes.GLARE_LOCATING_GLOW_BERRIES_COOLDOWN.get(), MemoryModuleState.VALUE_PRESENT)
			)
		);
	}
	...
	public static void updateMemories(GlareEntity glare) {
		if (
			...
			&& glare.getBrain().isMemoryInState(LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_LOCATING_COOLDOWN.get(), MemoryModuleState.VALUE_ABSENT)
		) {
		...
		if (
			...
			glare.getBrain().remember(LostLegendsMemoryModuleTypes.GLARE_IS_IDLE.get(), true);
		} else {
			glare.getBrain().forget(LostLegendsMemoryModuleTypes.GLARE_IS_IDLE.get());
		}

		if (glare.isTamed()) {
			glare.getBrain().remember(LostLegendsMemoryModuleTypes.GLARE_IS_TAMED.get(), true);
		} else {
			glare.getBrain().forget(LostLegendsMemoryModuleTypes.GLARE_IS_TAMED.get());
		}
	}

	public static void setDarkSpotLocatingCooldown(GlareEntity glare) {
		glare.getBrain().remember(LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_LOCATING_COOLDOWN.get(), DARK_SPOT_LOCATING_COOLDOWN_PROVIDER.get(glare.getRandom()));
	}

	public static void setDarkSpotLocatingCooldown(GlareEntity glare, UniformIntProvider cooldown) {
		glare.getBrain().remember(LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_LOCATING_COOLDOWN.get(), cooldown.get(glare.getRandom()));
	}

	public static void setLocatingGlowBerriesCooldown(GlareEntity glare) {
		glare.getBrain().remember(LostLegendsMemoryModuleTypes.GLARE_LOCATING_GLOW_BERRIES_COOLDOWN.get(), EAT_GLOW_BERRIES_COOLDOWN_PROVIDER.get(glare.getRandom()));
	}

	public static void setLocatingGlowBerriesCooldown(GlareEntity glare, UniformIntProvider cooldown) {
		glare.getBrain().remember(LostLegendsMemoryModuleTypes.GLARE_LOCATING_GLOW_BERRIES_COOLDOWN.get(), cooldown.get(glare.getRandom()));
	}
	...
	public static Ingredient getTemptItems() {
		return Ingredient.fromTag(LostLegendsTags.GLARE_TEMPT_ITEMS);
	}
	...
	static {
		SENSORS = List.of(
			...
			LostLegendsMemorySensorType.GLARE_TEMPTATIONS.get(),
			LostLegendsMemorySensorType.GLARE_SPECIFIC_SENSOR.get()
		);
		MEMORY_MODULES = List.of(
			...
			LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_POS.get(),
			LostLegendsMemoryModuleTypes.GLARE_GLOW_BERRIES_POS.get(),
			LostLegendsMemoryModuleTypes.GLARE_IS_IDLE.get(),
			LostLegendsMemoryModuleTypes.GLARE_IS_TAMED.get(),
			LostLegendsMemoryModuleTypes.GLARE_DARK_SPOT_LOCATING_COOLDOWN.get(),
			LostLegendsMemoryModuleTypes.GLARE_LOCATING_GLOW_BERRIES_COOLDOWN.get()
		);
