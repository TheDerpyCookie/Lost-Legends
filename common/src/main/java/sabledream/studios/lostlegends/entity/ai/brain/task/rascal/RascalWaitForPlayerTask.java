package sabledream.studios.lostlegends.entity.ai.brain.task.rascal;
...
public final class RascalWaitForPlayerTask extends MultiTickTask<RascalEntity>
{
	...
	public RascalWaitForPlayerTask() {
		super(ImmutableMap.of(
			...
			LostLegendsMemoryModuleTypes.RASCAL_NOD_COOLDOWN.get(), MemoryModuleState.VALUE_ABSENT
		), NOD_DURATION);
	}
	...
	protected void keepRunning(ServerWorld world, RascalEntity rascal, long time) {
		...
		if (nodTicks == 62 && rascal.shouldGiveReward()) {
			...
			LootTable rascalGoodItemsLootTable = world.getServer().getReloadableRegistries().getLootTable(RegistryKey.of(RegistryKeys.LOOT_TABLE, LostLegends.makeID("rewards/rascal_good_reward")));
			...
			for (ItemStack rascalReward : rascalGoodRewards) {
				...
				LostLegendsCriteria.COMPLETE_HIDE_AND_SEEK_GAME.get().trigger((ServerPlayerEntity) this.nearestTarget, rascal, bundleItemStack);
			}
		}
